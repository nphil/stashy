...
///
/// Concurrency: intentionally **non-isolated**. `interpolate(...)` builds its VideoToolbox parameter
/// objects and awaits `process` all within one isolation domain, so no non-`Sendable` value crosses an
/// actor boundary inside here. The caller (Phase 1b) owns single-threaded use — drive it from one context.
final class SlowMoInterpolator {
