...
| Auto ThumbHash new scenes (on scan) | bool | on | The same `Scene.Create`/`Scene.Destroy` hooks also keep the served ThumbHash map current — new scenes auto-hashed (a light ffmpeg cover decode), deleted ones pruned; no scene writes. Independent of the codec-report toggle. |
